---
agent: agent
description: "유저의 명령에 따라 작업 명령서 작성"
---

ORDER 파일에 따라 구현을 수행한다.

## 사전 확인

- [ ] `git status` 클린 상태 확인
- [ ] feature 브랜치 생성 및 체크아웃
- [ ] ORDER 파일이 사용자 승인 완료 상태인지 확인

## 실행 순서

### 1. ORDER 파일 선택

`/docs/current_sprint/`에서 최신 ORDER 파일을 선택한다.

- 정렬 기준: MMDD 내림차순 → NN 내림차순
- 예: order122403.md > order122402.md > order122301.md

### 2. 실행 모드 결정

| 모드 | 조건 | 동작 |
|------|------|------|
| 기본 | 특별 지시 없음 | 수동 테스트 필요 시점까지 진행 후 사용자에게 테스트 요청 |
| 특정 Phase | "Phase N까지" 지시 | 해당 Phase까지만 진행 |
| 전체 | "전체 구현" 지시 | 모든 Phase 완료 |

### 3. Phase별 구현

각 Phase마다:

1. 구현 수행
2. 자동화 테스트 실행 (API 테스트 등 해당 시)
3. ORDER 파일 체크리스트 업데이트
4. 수동 테스트 필요 시 → 사용자에게 요청 후 대기

### 4. 실패 시 행동

| 상황 | 행동 |
|------|------|
| 테스트 실패 | 즉시 중단, 원인 분석 후 보고 |
| 구현 중 에러 | 중단, 에러 보고 형식에 따라 보고 |
| 요구사항 불명확 | 중단, 사용자에게 질문 |

### 5. 완료 처리

모든 구현 완료 시:

1. ORDER 파일에 결과 기록
   - 리스크 및 롤백: 발생한 이슈
   - TODO: 추가 작업 항목
2. 사용자에게 완료 보고
3. 커밋은 전체 완료 후 1회 수행 (ORDER 종료 커맨드에서 처리)

> 이후 ORDER 종료 커맨드를 통해 커밋 및 브랜치 정리를 진행한다.